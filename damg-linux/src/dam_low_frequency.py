...
class OracleManage(object):
    ...
    def obtain_version(self):
        ...
        handle_path = 'su - oracle -c \'{path}\''.\
            format(path=script_path)
        return_info = handle_motion(motion_info=handle_path,
                                    logger_inst=log)
    ...
    def obtain_segment_num(self):
        ...
        # Make the shell script path.
        script_name = '.show_segment_num.sh'
        script_path = self.oracle_home + script_name
    ...
    def obtain_obj_type_num(self):
        ...
        # Make the shell script path.
        script_name = '.show_obj_typ_num.sh'
        script_path = self.oracle_home + script_name
    ...
    def obtain_wait_event(self):
        ...
        # Make the shell script path.
        script_name = '.show_wait_event.sh'
        script_path = self.oracle_home + script_name
    ...
    def obtain_health(self):
        ...
        # Make the shell script path.
        script_name = '.show_instance_name.sh'
        script_path = self.oracle_home + script_name
        ...
        handle_path = 'su - oracle -c \'{path}\''.\
            format(path=script_path)
        instance_name = handle_motion(motion_info=handle_path,
                                      logger_inst=log)
        ...
        # Make the shell script path.
        script_name = '.show_trace_path.sh'
        script_path = self.oracle_home + script_name
        ...
        handle_path = 'su - oracle -c \'{path}\''.\
            format(path=script_path)
        trace_path = handle_motion(motion_info=handle_path,
                                   logger_inst=log)
...
class SystemManage(object):
    ...
    def obtain_instance_num(self):
        ...
        # Get the oracle instance number.
        obtain_info = 'ps -ef |e' \
                      'grep -v \'ASM|grep\'' \
                      ' |grep -c p' \
                      'mon'
        ...
        ret = handle_motion(motion_info=obtain_info,
                            logger_inst=log)
        if ret:
            _COLLECT_NUM += 1
...
class ModuleManage(object):
    ...
    def obtain_object(self):
        ...
        oracle_inst.update({"session_max": session_num})
        ...
        oracle_inst.update({"process_max": process_num})
        ...
        for usr in oracle_user:
            ...
            for typ in object_type:
                single_type = typ.decode('UTF-8').rstrip()

                object_name_list = []
                ret = ocm.obtain_object_name(single_type)
                for obj in ret:
                    single_info = obj.decode('UTF-8').rstrip()
        ...
        for eve in wait_event:
            single_info = eve.decode('UTF-8').rstrip()
